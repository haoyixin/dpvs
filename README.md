...
Currently, `dpdk-stable-17.11.1` is used for `DPVS`.
...
```bash
$ wget https://fast.dpdk.org/rel/dpdk-17.11.1.tar.xz   # download from dpdk.org if link failed.
$ tar vxf dpdk-17.11.1.tar.xz
```

There's a patch for DPDK `kni` driver for hardware multicast, apply it if needed (for example, launch `ospfd` on `kni` device).

> assuming we are in DPVS root dir and dpdk-stable-17.11.1 is under it, pls note it's not mandatory, just for convenience.

```
$ cd <path-of-dpvs>
$ cp patch/dpdk-stable-17.11.1/0001-PATCH-kni-use-netlink-event-for-multicast-driver-par.patch dpdk-stable-17.11.1/
...
In our tutorial, `RTE_TARGET` is not set, the value is "build" by default, thus DPDK libs and header files can be found in `dpdk-stable-17.11.1/build`.
